...
# Create the new index based on the specified format so it is the same as log files
new_index = [
...
#load memory measurements from pickle file
mem_outch=pd.read_pickle('mem_outch.pkl')
...
plt.savefig('C:/Users/demib/Documents/Thesis/Data_final_processing/Figures/Laptop/lp_eng_outch.pdf', format='pdf')
...
# Save the plot as an SVG file
svg_filename = 'C:/Users/demib/Documents/Thesis/Data_final_processing/Figures/Laptop/lp_mem_outch.pdf'
...
def plot_method_with_inset(ax, data, method_name):
    for i, channel in enumerate(channels):
        subset = data[data['Out_ch'] == channel]
        if not subset.empty:
            sns.lineplot(x='Comp', y='Rank', data=subset, ax=ax, label=f'Out_ch={channel}', zorder=3, color=colors[i])
            ax.scatter(subset['Comp'], subset['Rank'], zorder=4, color=colors[i])
    ax.set_ylim([0, 1100])
    if method_name=='CP':
        ax.set_ylabel('R', fontsize=14)
    elif method_name=='TT':
        ax.set_ylabel('R3', fontsize=14)
    else:
        ax.set_ylabel('R1', fontsize=14)
    ax.set_title(f'Ranks for Mode-T {method_name} Decomposition',fontsize=14)
...
svg_filename = 'C:/Users/demib/Documents/Thesis/Data_final_processing/Figures/Ranks/rankT_combined.pdf'
plt.savefig(svg_filename, format='pdf')
plt.show()
