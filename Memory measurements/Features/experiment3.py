...
def run_model(x,cnn_dict, fact_dict):
    ...
    # Calculate total inclusive memory usage
    key_averages = prof.key_averages()
    print('d')
    total_inclusive_memory = sum([item.cpu_memory_usage for item in key_averages])
    print(f"Total inclusive memory usage: {total_inclusive_memory / (1024 * 1024):.2f} MB")
    
    return(model,total_inclusive_memory)
...
for img_h in [2,4]:
    ...
    with open(f'inch{in_chan}-wh{img_h}.pkl','rb') as f:  
        x = pickle.load(f)
    ...
    for method in methods:
        if method=='nd':
            ...
            for ind in [3]:
                ...
                mem_dict[key]={'Mem': np.round(mem,decimals=3)}
