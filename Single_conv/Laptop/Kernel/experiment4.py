...
if not any(isinstance(handler, logging.FileHandler) for handler in logger.handlers):
    fh = logging.FileHandler('Kernel_ind3.log')
...
def run_model(x,cnn_dict, fact_dict):
    ...
    if decompose==True:
        logger.info(f"dec-start-outch{out_channels}-inch{in_channels}-fact{factorization}-r{rank}-wh{img_w}-kern{kernel_size}-ind{ind}s")
    else:
        logger.info(f"bas-start-outch{out_channels}-inch{in_channels}-wh{img_w}-kernel{kernel_size}-ind{ind}s")
    with torch.no_grad():
    ...
    if decompose==True:
        ...
        logger.info(f"bas-end-outch{out_channels}-inch{in_channels}-wh{img_w}-kernel{kernel_size}-ind{ind}s")
